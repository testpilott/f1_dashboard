...
describe("buildICS", () => {
  it("wraps events in a VCALENDAR with CRLF endings", () => {
    const ics = buildICS([makeRace({ round: "1", raceName: "Italian GP", date: "2026-09-06", time: "13:00:00Z" })], { now: NOW });
  ...
  it("emits a timed 2-hour event when a start time is present", () => {
    const ics = buildICS(
      [makeRace({ round: "16", raceName: "Italian GP", date: "2026-09-06", time: "13:00:00Z" })],
      { now: NOW },
    );
  ...
  it("emits an all-day event when no start time is given", () => {
    const ics = buildICS([makeRace({ round: "2", raceName: "Test GP", date: "2026-03-15", time: undefined })], { now: NOW });
  ...
  it("escapes punctuation in SUMMARY/LOCATION", () => {
    const ics = buildICS(
      [makeRace({ round: "1", raceName: "São Paulo GP; round, one", date: "2026-11-08", time: "17:00:00Z" })],
      { now: NOW },
    );
